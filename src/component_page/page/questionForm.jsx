...
function QuestionForm() {
  ...
  const validateAnswers = () => {
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.loai === "true_false" && !["true", "false"].includes(answer.toLowerCase())) {
        return `Câu ${i + 1} chỉ chấp nhận 'True' hoặc 'False'!`;
      }
      if (q.loai === "short_answer" && (!answer.startsWith("[") || !answer.endsWith("]"))) {
      ...
      {questions.length > 0 ? (
        <form onSubmit={handleSubmit}>
          <div>
            ...
            <input type="text" value={maNguoiChoi} onChange={(e) => setMaNguoiChoi(e.target.value)} required />
          </div>
          <div>
          ...
          {questions.map((question, index) => (
            <div key={index}>
              <p><strong>Câu {index + 1}:</strong> {question.cauHoi}</p>
              {question.loai === "true_false" ? (
                <input type="text" value={answers[question.cauHoi] || ""} onChange={(e) => handleAnswerChange(question.cauHoi, e.target.value)} required />
              ) : (
                <textarea value={answers[question.cauHoi] || ""} onChange={(e) => handleAnswerChange(question.cauHoi, e.target.value)} required />
              )}
            </div>
          ))}
        ...
        <div>
          ...
          <ul>
            {result.ketQua.map((item, index) => (
              <li key={index}>
                <p><strong>Câu hỏi:</strong> {item.cauHoi}</p>
                <p><strong>Câu trả lời của bạn:</strong> {item.cauTraLoi}</p>
                <p><strong>Kết quả:</strong> {item.isTrue ? "Đúng" : "Sai"}</p>
              </li>
            ))}
          </ul>
        </div>
